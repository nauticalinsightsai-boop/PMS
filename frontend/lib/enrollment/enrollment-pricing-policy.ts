/**
 * Single source of truth for enrollment pay fractions.
 * Stated offs are customer-facing; pay fractions silently embed PROCESSING_FEE_FRACTION.
 * Never surface the fee, effective offs, or raw pay math in UI copy.
 */

import type { RegionId, TierId } from '@/types/regional-catalogue';
...
export function statedRegionalOff(tierId: TierId | string, regionId: RegionId | string): number {
...
export function regionalPayFraction(tierId: TierId | string, regionId: RegionId | string): number {
...
export function statedRegionalOffPercent(tierId: TierId | string, regionId: RegionId | string): number {
