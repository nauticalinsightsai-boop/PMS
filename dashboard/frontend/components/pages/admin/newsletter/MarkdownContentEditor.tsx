...
export function MarkdownContentEditor({
  ...
  // Re-apply caret/selection after a controlled value update from a toolbar action.
  useEffect(() => {
  ...
  /** Wrap the current selection with inline markers (bold, italic, code, …). */
  ...
  /** Apply a per-line prefix (headings, quote, lists). */
  const prefixLines = (
  ...
  > = [
    ...
    { kind: 'btn', label: 'Italic', icon: <Italic size={15} />, onClick: () => wrapInline('*', 'italic text') },
    {
      ...
      onClick: () => wrapInline('~~', 'struck text'),
    },
    ...
    { kind: 'divider' },
    { kind: 'btn', label: 'Quote', icon: <Quote size={15} />, onClick: () => prefixLines(() => '> ') },
    {
      kind: 'btn',
      label: 'Bulleted list',
      icon: <List size={15} />,
    ...
    },
    {
      ...
      onClick: () => prefixLines((i) => `${i + 1}. `, { stripHeading: true }),
    },
    { kind: 'divider' },
