    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground font-medium uppercase tracking-widest text-[10px] flex items-center gap-2">
            <FileText size={12} className="text-brand-orange" /> Content Management System
          </p>
        </div>
      ...
      <GlassCard className="p-4 flex flex-col md:flex-row items-center gap-4">
        <div className="relative flex-1 group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-brand-orange transition-colors" size={18} />
          <input 
            type="text"
            placeholder="Search articles, categories, or keywords..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-brand-orange transition-all outline-none"
          />
        </div>
            ...
            <GlassCard variant="surface" className="p-6 hover:border-white/20 transition-all group">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                <div className="w-full md:w-32 aspect-video bg-white/5 rounded-xl flex items-center justify-center overflow-hidden relative">
                   <Layout size={24} className="text-muted-foreground opacity-30 group-hover:scale-110 transition-transform" />
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <h3 className="text-lg font-bold group-hover:text-brand-orange transition-colors">{post.title}</h3>
                ...
                <div className="flex items-center gap-2 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-white/5">
                  <button className="flex-1 md:flex-none p-2 rounded-xl bg-white/5 hover:bg-white/10 text-muted-foreground transition-colors" title="Preview">
                    <Eye size={18} />
                  </button>
                  <button className="flex-1 md:flex-none p-2 rounded-xl bg-white/5 hover:bg-white/10 text-brand-orange transition-colors" title="Edit">
                    <Edit3 size={18} />
                  </button>
      ...
      {filteredPosts.length === 0 && (
        <div className="py-20 text-center">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-white/5 text-muted-foreground mb-6">
            <Search size={40} />
          </div>
