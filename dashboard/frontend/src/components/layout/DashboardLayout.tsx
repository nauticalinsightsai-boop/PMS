          ...
          {currentRoutes.map((section) => (
            <div key={section.title} className="space-y-1">
              ...
              {section.items.map((item) => (
                <div key={item.path}>
                  <NavLink to={item.path} className={navItemClasses} end={item.path.endsWith('/dashboard')}>
                    <item.icon size={20} className={cn(!isSidebarCollapsed && "mr-3")} />
                    {!isSidebarCollapsed && <span>{item.name}</span>}
                  </NavLink>
                  {item.subItems && !isSidebarCollapsed && location.pathname.startsWith(item.path) && (
                      ...
                      {item.subItems.map((sub) => (
                        <NavLink 
                          ...
                          className={({ isActive }) => cn(
                            ...
                            isActive ? "text-gw-accent-primary" : "text-gw-text-secondary hover:text-gw-text-primary"
                          )}
                        >
                          {sub.name}
                        </NavLink>
        ...
        <header className="h-[75px] flex items-center justify-between px-4 md:px-8 border-b border-white/5 bg-gw-bg-secondary/50 backdrop-blur-xl sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center">
               <Link to="/dashboard" className="lg:hidden flex items-center gap-2 mr-4">
                <BrandLogo />
              </Link>
            </div>
          </div>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-2 md:gap-8 text-[10px] font-bold uppercase tracking-widest text-gw-text-secondary">
                  ...
                  <div className="flex items-center gap-6">
                    <Link to="/dashboard/site-system/security" className="hover:text-gw-accent-primary transition-colors">Security Audit</Link>
                    <Link to="/dashboard/site-system/settings" className="hover:text-gw-accent-primary transition-colors">Platform Config</Link>
                    <Link to="/dashboard/site-system/analytics" className="hover:text-gw-accent-primary transition-colors">System Logs</Link>
                  </div>
                </div>
              </div>
            </div>
